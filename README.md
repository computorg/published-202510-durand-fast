...
$V^{\ast}_{\mathfrak R}$ construction is based on a reference family ${\mathfrak R}$ with a forest structure à la Durand et al. (2020). By anentire curve, we mean the values
entire curve, we mean the values
$V^{\ast}_{\mathfrak R}(S_1),\dotsc,V^{*}_{\mathfrak R}(S_m)$ computed on a path of increasing selection sets $S_1\subsetneq\dotsb\subsetneq S_m$,
...
naive approach, the new algorithm has a complexity in $O(|\mathcal K|m)$
instead of $O(|\mathcal K|m^2)$, where $|\mathcal K|$ is the cardinality
of the family.
