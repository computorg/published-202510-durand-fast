...
\begin{center}
      \textbf{Guillermo
Durand}\textsuperscript{1}~\orcidlink{0000-0003-4056-5631}{\renewcommand{\thefootnote}{\fnsymbol{footnote}}\footnote{Corresponding author: \href{mailto:guillermo.durand@universite-paris-saclay.fr}{guillermo.durand@universite-paris-saclay.fr}}}\quad
  
  \smallskip
  {\small
      \textsuperscript{1}\href{https://www.universite-paris-saclay.fr/}{Université
Paris-Saclay}, \href{https://www.cnrs.fr}{CNRS},
\href{https://team.inria.fr/celeste/}{Inria},
\href{https://www.imo.universite-paris-saclay.fr}{Laboratoire de
Mathématiques d'Orsay}, 91405, Orsay, France\quad
    }

  \bigskip

  Date published: 2025-10-09 \quad Last modified: 2026-06-29
    
  \smallskip
  DOI: \href{https://doi.org/10.57750/efbs-ef14}{10.57750/efbs-ef14}
    \end{center}
...
leverages the fact that going from \(S_t\) to \(S_{t+1}\) is done by
adding only one hypothesis. Compared to a more naive approach, the new
...
which is reproduced here (see Algorithm~\ref{algo-vstar}) is fast for a
...
algorithm (Algorithm~\ref{algo-formal-curve}) computing the curve
...
results, including the proof that Algorithm~\ref{algo-formal-curve}
...
\pandocbounded{\includegraphics[keepaspectratio]{published-202510-durand-fast_files/figure-pdf/fig-benchmark_intro-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{published-202510-durand-fast_files/mediabag/../figure-tikz/fig-tikz-01.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{published-202510-durand-fast_files/mediabag/../figure-tikz/fig-tikz-02.pdf}}
...
which we reproduce here in Algorithm~\ref{algo-vstar}. The family is
...
\begin{algorithm}[htb!]
\caption{\label{algo-vstar}Computation of a given $V^*_{\mathfrak{R}}(S)$ with a complete family}
\begin{algorithmic}[1]
\Procedure{Vstar}{S, $\mathfrak{R}=(R_{k},\zeta_{k})_{k\in\mathcal{K}}$  with $\mathfrak{R}$ complete}
  ...
  \State $Vec\gets (\zeta_{k}\wedge|S\cap R_k|)_{k \in  \mathcal{K}^H}$ \Comment{initialization}
  \For{$h = H-1, \dotsc, 1$}
...
A step by step description of Algorithm~\ref{algo-vstar} is provided at
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-tip-color!10!white, colframe=quarto-callout-tip-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Tip \ref*{tip-algo1}}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

\quartocallouttip{tip-algo1} 

In the practical implementation of this algorithm (and of the following
Algorithm~\ref{algo-pruning}), \(Vec\) and \(newVec\) are always of size
\(N\) (the number of leaves) instead of the cardinality of
...
of Algorithm~\ref{algo-vstar}: it is in
...
Algorithm~\ref{algo-vstar} for \(S=\mathbb{N}_m^*\), and pruning when
appropriate. This gives the following Algorithm~\ref{algo-pruning},
assuming, for simplicity, that the family is complete. Note that the
only differences between Algorithm~\ref{algo-pruning} and
Algorithm~\ref{algo-vstar} are the pruning step and \(\zeta_k\)
...
\restylefloat{algorithm}
\begin{algorithm}[htb!]
\caption{\label{algo-pruning}Pruning of a complete $\mathfrak{R}$}
\begin{algorithmic}[1]
...
proposition states that Algorithm~\ref{algo-pruning} indeed produces the
pruned region as in Definition~\ref{def-pruning}.

\begin{proposition}[]\protect\hypertarget{prp-pruning-correct}{}\label{prp-pruning-correct}

The final \(\mathcal{L}\) returned by Algorithm~\ref{algo-pruning} is
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-tip-color!10!white, colframe=quarto-callout-tip-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Tip \ref*{tip-pruning-complexity}}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

\quartocallouttip{tip-pruning-complexity} 

We saw that Algorithm~\ref{algo-vstar} has \(O(N|S|+|\mathcal{K}|)\)
complexity, the \(N|S|\) term coming from the evaluation of the
\(|S\cap P_i|\) terms, \(1\leq i\leq N\). Here,
\(|S\cap P_i|=|\mathbb{N}_m^*\cap P_i|=|P_i|\) can be accessed in
\(O(1)\), and \(N\leq |\mathcal{K}|\) for a complete family, so
Algorithm~\ref{algo-pruning} simply has \(O(|\mathcal{K}|)\) complexity.
...
\((i_1,\dotsc, i_m).\) Applying Algorithm~\ref{algo-vstar} to compute
...
(Algorithm~\ref{algo-formal-curve}), to introduce additional notation
used in the proof of Theorem~\ref{thm-curve-path}, and, later, a simpler
version that is the one actually implemented
(Algorithm~\ref{algo-curve}). Recall that a detailed illustration of the
steps of the algorithms will be provided in Section~\ref{sec-example}.

In addition to the computation of all \(V^*_{\mathfrak{R}}(S_t)\),
Algorithm~\ref{algo-formal-curve} also computes partitions
...
\restylefloat{algorithm}
\begin{algorithm}[htb!]
\caption{\label{algo-formal-curve}Formal computation of $(V^*_{\mathfrak{R}}(S_t))_{0\leq t\leq m}$ with a complete family}
\begin{algorithmic}[1]
...
Algorithm~\ref{algo-curve}. Note that Algorithm~\ref{algo-curve} is less
formal than Algorithm~\ref{algo-formal-curve}: as in
Algorithm~\ref{algo-vstar} and Algorithm~\ref{algo-pruning}, it recycles
notation (mimicking the actual code implementation) so the \(t\)
subscript or superscript is dropped from all \(\mathcal{K}^-_t\) and
\(\eta_k^t\). In Algorithm~\ref{algo-curve}, the notation \(V_t\) is
actually equal to \(V^*_{\mathfrak{R}}(S_{t})\) by
Corollary~\ref{cor-easy-impl}.

\floatstyle{ruled}
\restylefloat{algorithm}
\begin{algorithm}[htb!]
\caption{\label{algo-curve}Practical computation of $(V^*_{\mathfrak{R}}(S_t))_{0\leq t\leq m}$}
\begin{algorithmic}[1]
\Procedure{Curve}{$\mathfrak{R}=(R_{k},\zeta_{k})_{k\in\mathcal{K}}$ with $\mathfrak{R}$ complete, path $(S_t)_{1\leq t \leq m}$ with $S_t=\{i_1, \dotsc, i_t\}$}
  \State $V_0\gets 0$
...
Algorithm~\ref{algo-curve} is in
...
In this section, we follow Algorithm~\ref{algo-formal-curve} during its
...
\pandocbounded{\includegraphics[keepaspectratio]{published-202510-durand-fast_files/mediabag/../figure-tikz/fig-tikz-03.pdf}}
...
First, we apply Algorithm~\ref{algo-pruning} to the family. This results
in pruning \(P_{6:7}\) (and only this region), because
...
\pandocbounded{\includegraphics[keepaspectratio]{published-202510-durand-fast_files/mediabag/../figure-tikz/fig-tikz-04.pdf}}
...
Now we initialize Algorithm~\ref{algo-formal-curve}, that is we let
...
The initial state of Algorithm~\ref{algo-formal-curve} is shown in
...
\pandocbounded{\includegraphics[keepaspectratio]{published-202510-durand-fast_files/mediabag/../figure-tikz/fig-tikz-05.pdf}}

}

\caption{\label{fig-t0}The regions of Example~\ref{exm-toy-forest} at
\(t=0\) in Algorithm~\ref{algo-formal-curve}.}
...
\pandocbounded{\includegraphics[keepaspectratio]{published-202510-durand-fast_files/mediabag/../figure-tikz/fig-tikz-06.pdf}}

}

\caption{\label{fig-t1}The regions of Example~\ref{exm-toy-forest} at
\(t=1\) in Algorithm~\ref{algo-formal-curve}.}
...
\pandocbounded{\includegraphics[keepaspectratio]{published-202510-durand-fast_files/mediabag/../figure-tikz/fig-tikz-07.pdf}}

}

\caption{\label{fig-t2}The regions of Example~\ref{exm-toy-forest} at
\(t=2\) in Algorithm~\ref{algo-formal-curve}.}
...
of Algorithm~\ref{algo-formal-curve}, \(\mathcal{P}^t\) does not change
...
\pandocbounded{\includegraphics[keepaspectratio]{published-202510-durand-fast_files/mediabag/../figure-tikz/fig-tikz-08.pdf}}

}

\caption{\label{fig-t3}The regions of Example~\ref{exm-toy-forest} at
\(t=3\) in Algorithm~\ref{algo-formal-curve}.}

\end{figure}%

We move on to \(t=4\), with \(i_4=13\).
\(i_4\in P_4\in \bigcup_{k\in\mathcal{K}^-_3}R_k\). No \(\eta_k^t\) is
increased (see line 9 of Algorithm~\ref{algo-formal-curve}), and by
...
before, see line 23 of Algorithm~\ref{algo-formal-curve}. By
...
\pandocbounded{\includegraphics[keepaspectratio]{published-202510-durand-fast_files/mediabag/../figure-tikz/fig-tikz-09.pdf}}

}

\caption{\label{fig-t5}The regions of Example~\ref{exm-toy-forest} at
\(t=5\) in Algorithm~\ref{algo-formal-curve}.}
...
\pandocbounded{\includegraphics[keepaspectratio]{published-202510-durand-fast_files/mediabag/../figure-tikz/fig-tikz-10.pdf}}

}

\caption{\label{fig-t6}The regions of Example~\ref{exm-toy-forest} at
\(t=6\) in Algorithm~\ref{algo-formal-curve}.}
...
their step (see line 9 of Algorithm~\ref{algo-formal-curve}), and by
...
\citet{10.1093/bioinformatics/btac693}. Algorithm~\ref{algo-vstar} is
implemented as the \texttt{V.star} function,
Algorithm~\ref{algo-pruning} is implemented as the \texttt{pruning}
function, and Algorithm~\ref{algo-curve} is implemented as the
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-caution-color!10!white, colframe=quarto-callout-caution-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Caution \ref*{cau-impl}}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-tip-color!10!white, colframe=quarto-callout-tip-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Tip \ref*{tip-leaves}}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
implementations of Algorithm~\ref{algo-vstar} and
Algorithm~\ref{algo-pruning} given that, as stated in
...
for Algorithm~\ref{algo-curve}. Indeed, if \(R_k\) is a region such that
\(\zeta_k=|R_k|\), the condition of line 12 of
Algorithm~\ref{algo-curve} is always true, except for the last element
...
\FunctionTok{curve.V.star.forest.fast}\NormalTok{(o, complete.res}\SpecialCharTok{$}\NormalTok{C, complete.res}\SpecialCharTok{$}\NormalTok{ZL, leaf\_list)}
\end{Highlighting}
\end{Shaded}
...
demonstrate the impact of the pruning of Algorithm~\ref{algo-pruning}
(using the \texttt{delete.gaps} option mentioned in
Section~\ref{sec-implementation}) and of the fast
Algorithm~\ref{algo-curve}, in terms of computation time, compared to
...
a \texttt{for} loop repeatedly applying Algorithm~\ref{algo-vstar}.
...
\citet{MR4178188} with the DKWM inequality \citep{MR0083864, MR1062069}.
Because of the size of \(m\) and the poor performances of the naive
approach, we set \texttt{n\_repl=100} in scenarios 1 and 2 and
\texttt{n\_repl=10} only in scenarios 3 and 4. The differences between
the scenarios are summarized in Table~\ref{tbl-scenarios}.
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.2048}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1190}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1190}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1310}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1263}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1263}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1263}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1263}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1263}}
...
\pandocbounded{\includegraphics[keepaspectratio]{published-202510-durand-fast_files/figure-pdf/fig-benchmark01-1.pdf}}
...
computation time by \(\sim10\) when using Algorithm~\ref{algo-curve} and
by \(\sim100\) when using Algorithm~\ref{algo-vstar} naively, which
...
\((i,i')\) is pruned by Algorithm~\ref{algo-pruning} and
...
Algorithm~\ref{algo-formal-curve}.
...
[1] sanssouci_0.16.2     microbenchmark_1.5.0

loaded via a namespace (and not attached):
 [1] digest_0.6.39       fastmap_1.2.0       xfun_0.59          
 [4] Matrix_1.7-5        lattice_0.22-9      matrixStats_1.5.0  
 [7] knitr_1.51          htmltools_0.5.9     generics_0.1.4     
[10] rmarkdown_2.31      tinytex_0.60        cli_3.6.6          
[13] grid_4.6.1          renv_1.2.3          matrixTests_0.2.3.1
[16] compiler_4.6.1      tools_4.6.1         evaluate_1.0.5     
[19] yaml_2.3.12         rlang_1.2.0         jsonlite_2.0.0     
\end{verbatim}
